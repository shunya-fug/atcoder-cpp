...
int64_t solve(int64_t N, int M, const std::vector<int64_t> &A, const std::vector<int64_t> &B)
{
    ...
    REP(i, N)
    {
        ...
        auto search = [&infos, &known](auto self, int64_t index)
        {
            if (all_of(ALL(known), [](bool b)
                       { return b; }))
            {
                return;
            }
            ...
            for (auto info : infos)
            {
                if (info.winner == index)
                {
                    self(self, info.loser);
                }
            }
        };
        search(search, i);
    ...
    return winners.size() == 1 ? *winners.begin() : -1;
}
